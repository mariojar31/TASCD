...
import { View, Text, Button, TextInput, StyleSheet, SafeAreaView, Dimensions, Pressable } from 'react-native';
...
const ReflectionQuiz = () => {
  ...
  const Questions = [    
    ...
    const saveDevotional = async () => {
      const newItem = { name: `${time.getTime()}-${id}`, date: time.toISOString().slice(0, 10), answers: answers, commitments: commitments };
      try {
    ...
    <SafeAreaView         style={{
      ...
      {currentQuestionIndex === -1 ? (
        <View style={styles.introContainer}>
          ...
          <Text style={styles.introText}>En este espacio encontraras una serie de preguntas que te ayudarán a reflexionar a cerca del texto que acabas de leer. </Text>
        ...
        <View style={styles.introContainer}>
          <Text style={styles.thankYouTextHeader}>Haz finalizado tu tiempo a solas con Dios.</Text>
          ...
          <Pressable style={({pressed})=>pressed?styles.btnPressed:styles.Button} onPress={() => navigation.goBack()}><Text style={styles.btnText}>Amén</Text></Pressable>
        </View>
        ...
        <View style={styles.questionContainer}>
          <View style={{display:"flex", flexDirection:"row", justifyContent:"center"}}>
            ...
            <Tooltip
                ...
                >
                <Entypo name="help-with-circle" size={20} style={styles.topMenuIcons} />
            </Tooltip>
          </View>
